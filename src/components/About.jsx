import Link from "next/link";
export default function About() {
  return (
    <>
      <div className="container heading-about">
      <div className="display-6 mb-1 text-justify">
          <Link className="btn border-0  back  shadow btn-sm mx-1" href="/">
            <i className="bi bi-arrow-left mx-1"> </i>
          </Link>
        </div>
        
        <h1 className="mt-3 ms-1">About us</h1>
      </div>
      <div
        className="container p-4 shadow  justify-content-center  align-items-center"
        id="About"
      >
        <div className=" row">
          <p className="text-justify font-roboto">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi,
            similique cupiditate id excepturi ex voluptatibus delectus aliquam
            praesentium. Eligendi cupiditate sequi sit voluptates. Iure et sequi
            accusamus nulla delectus perspiciatis impedit ullam! Repudiandae
            earum aliquid doloremque accusamus? Dolore fuga ullam sapiente, hic
            impedit quas earum aliquam dignissimos modi harum itaque!
          </p>
       

          <p className="text-justify">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi,
            similique cupiditate id excepturi ex voluptatibus delectus aliquam
            praesentium. Eligendi cupiditate sequi sit voluptates. Iure et sequi
            accusamus nulla delectus perspiciatis impedit ullam! Repudiandae
            earum aliquid doloremque accusamus? Dolore fuga ullam sapiente, hic
            impedit quas earum aliquam dignissimos modi harum itaque!
          </p>
          <div className="h2 my-3 text-light">Brief History</div>
          <p className="text-justify">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi,
            similique cupiditate id excepturi ex voluptatibus delectus aliquam
            praesentium. Eligendi cupiditate sequi sit voluptates. Iure et sequi
            accusamus nulla delectus perspiciatis impedit ullam! Repudiandae
            earum aliquid doloremque accusamus? Dolore fuga ullam sapiente, hic
            impedit quas earum aliquam dignissimos modi harum itaque!
          </p>
          <p className="text-justify">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi,
            similique cupiditate id excepturi ex voluptatibus delectus aliquam
            praesentium. Eligendi cupiditate sequi sit voluptates. Iure et sequi
            accusamus nulla delectus perspiciatis impedit ullam! Repudiandae
            earum aliquid doloremque accusamus? Dolore fuga ullam sapiente, hic
            impedit quas earum aliquam dignissimos modi harum itaque!
          </p>
        </div>
     
     
        <section>
          <h1 className="my-3">
            Purpose of being a Computer Scientist Student
          </h1>
          <p className="text-justify">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi,
            similique cupiditate id excepturi ex voluptatibus delectus aliquam
            praesentium. Eligendi cupiditate sequi sit voluptates. Iure et sequi
            accusamus nulla delectus perspiciatis impedit ullam! Repudiandae
            earum aliquid doloremque accusamus? Dolore fuga ullam sapiente, hic
            impedit quas earum aliquam dignissimos modi harum itaque!
          </p>
          <h1 className="my-3">
            What you meant to achieve at the end of your year
          </h1>
          <p className="text-justify">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi,
            similique cupiditate id excepturi ex voluptatibus delectus aliquam
            praesentium. Eligendi cupiditate sequi sit voluptates. Iure et sequi
            accusamus nulla delectus perspiciatis impedit ullam! Repudiandae
            earum aliquid doloremque accusamus? Dolore fuga ullam sapiente, hic
            impedit quas earum aliquam dignissimos modi harum itaque!
          </p>
        </section>
        </div>
    </> 
  );
}
